...
namespace FluentHttpClient.Entity
{
	public static class ServiceCollectionExtensions
	{
		public static IServiceCollection AddFluentHttpClientEntity(this IServiceCollection services, string connectionString = "")
		{
			...
			var connection = @"Data Source=.\SQLEXPRESS;Database=FluentHttpClient;Integrated Security=True";

			if (connectionString != string.Empty)
				connection = connectionString;

			return services.AddDbContext<FluentHttpClientContext>(options => options.UseSqlServer(connection));
		}
	}
}